                       ...
                       struct str_parms *query, struct str_parms *reply)
{
    ...
    err = str_parms_get_str(query, AUDIO_PARAMETER_KEY_FLUENCE, value,
                                                          sizeof(value));
	ALOGE("%s: Retrieved value for %s in audio_extn_get_fluence_parameters1: %s", __func__,AUDIO_PARAMETER_KEY_FLUENCE,value);
    strcpy(value,"fluence");
    if (err >= 0) {
	...
	ALOGE("%s: Retrieved value for %s in audio_extn_get_fluence_parameters2: %s", __func__,AUDIO_PARAMETER_KEY_FLUENCE,value);
    strcpy(value,"fluence");
        } else
            goto done;
    }
