...
void *platform_init(struct audio_device *adev)
{
	...
	ALOGE("%s: Retrieved value for %s: %s", __func__,"ro.qc.sdk.audio.fluencetype",my_data->fluence_cap);
...
ALOGE("%s: fluence %s for %s", __func__, value, "voicecall");
        if (!strncmp("true", value, sizeof("true"))) {
            my_data->fluence_in_voice_call = true;
        }

        property_get("persist.audio.fluence.voicerec",value,"");
ALOGE("%s: fluence %s for %s", __func__, value, "voicerec");
        if (!strncmp("true", value, sizeof("true"))) {
            my_data->fluence_in_voice_rec = true;
        }

        property_get("persist.audio.fluence.audiorec",value,"");
ALOGE("%s: fluence %s for %s", __func__, value, "audiorec");
        if (!strncmp("true", value, sizeof("true"))) {
            my_data->fluence_in_audio_rec = true;
        }

        property_get("persist.audio.fluence.speaker",value,"");
ALOGE("%s: fluence %s for %s", __func__,value, "speaker");
        if (!strncmp("true", value, sizeof("true"))) {
